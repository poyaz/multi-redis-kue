...
function get( q, fn, order, jobType) {
  return function( err, ids ) {
    ...
    ids.forEach(function( id ) {
      id = client.stripFIFO(id); // turn zid back to regular job id
...
exports.range = function( q, from, to, order, fn ) {
  var client = redis.client();
  client.select(q._options.redis.db);
  client.zrange(client.getKey('jobs'), from, to, get(q, fn, order));
};
...
exports.rangeByState = function( q, state, from, to, order, fn ) {
  var client = redis.client();
  client.select(q._options.redis.db);
  client.zrange(client.getKey('jobs:' + state), from, to, get(q, fn, order));
};
...
exports.rangeByType = function( q, type, state, from, to, order, fn ) {
  var client = redis.client();
  client.select(q._options.redis.db);
  client.zrange(client.getKey('jobs:' + type + ':' + state), from, to, get(q, fn, order, type));
};
...
exports.get = function( q, id, jobType, fn ) {
  ...
  client.hgetall(client.getKey('job:' + job.id), function( err, hash ) {
    if( err ) return fn(err);
    if( !hash ) {
      exports.removeBadJob(q, job.id, jobType);
...
exports.removeBadJob = function( q, id, jobType) {
  var client = redis.client();
  client.select(q._options.redis.db);
...
exports.log = function( id, fn ) {
  /*redis*/
  Job.client.select(this.q._options.redis.db);
...
//  this.client = redis.client();
  ...
  this.client.select(q._options.redis.db);
...
Job.prototype.refreshTtl = function() {
  this.client.select(this.q._options.redis.db);
...
Job.prototype.progress = function( complete, total, data ) {
  ...
  this.set('updated_at', Date.now());
  this.refreshTtl();
  events.emit(this.q._options.redis.db, this.id, 'progress', n, data);
  return this;
};
...
Job.prototype.attempt = function( fn ) {
  ...
  client.select(this.q._options.redis.db);
  var id     = this.id
...
Job.prototype.remove = function( fn ) {
  ...
  client.multi()
    ...
    .exec(function( err ) {
//            events.remove(this);
      events.emit(this.q._options.redis.db, this.id, 'remove', this.type);
      if( !exports.disableSearch ) {
...
Job.prototype.state = function( state, fn ) {
  ...
  multi.exec(function( err, replies ) {
    if( !err ) {
      (this._state === 'inactive') ? events.emit(this.q._options.redis.db, this.id, 'enqueue', this.type) : noop();
    }
    return fn(err);
  }.bind(this));
  return this;
...
Job.prototype.error = function( err ) {
  ...
  this.set('error', str);
  this.log('%s', summary);
  events.emit(this.q._options.redis.db, this.id, 'error', str);
  return this;
};
