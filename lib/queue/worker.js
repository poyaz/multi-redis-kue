...
Worker.prototype.start = function( fn ) {
   ...
   */
  this.queue.on( 'job ttl exceeded', this.queue._options, function( id ) {
    if( this.job && this.job.id && this.job.id === id ) {
      //this.start(fn);
      this.failed( this.job, { error: true, message: 'TTL exceeded' }, fn );
      events.emit(this.queue._options.redis.db, id, 'ttl exceeded ack');
    }
  }.bind( this ));
...
Worker.prototype.zpop = function( key, fn ) {
  this.client.select(this.queue._options.redis.db);
  this.client
    .multi()
    .zrange(key, 0, 0)
...
Worker.prototype.getJob = function( fn ) {
  ...
  client.select(this.queue._options.redis.db);
...
Worker.prototype.emitJobEvent = function( event, job, arg1, arg2 ) {
  ...
  events.emit(this.queue._options.redis.db, job.id, event, arg1, arg2);
