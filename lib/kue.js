...
exports.createQueue = function (options) {
    ...
    q[options.name] = new Queue(options);
    Queue.singleton[options.name] = q[options.name];
    events.subscribe({_options: options});
    return q[options.name];
};
...
function Queue(options) {
    options = options || {};
    this.q = q[options.name] || 'kue' + Math.floor((Math.random() * 1000) + 1);
    this.name = options.name;
    this.id = ['kue' + Math.floor((Math.random() * 1000) + 1), require('os').hostname(), process.pid].join(':');
...
Queue.prototype.on = function (event, q) {console.log('event', event)
    var arg = []
    if (arguments.length == 2) {
        ...
        arg.push(arguments[0]);
        arg.push(arguments[2]);
    }
    if (0 == event.indexOf('job')) events.subscribe({_options: q});
    return on.apply(this, arg);
};
...
Queue.prototype.checkJobPromotion = function (promotionOptions) {
    ...
    client.select(this._options.redis.db);
    this.promoter = setInterval(function () {
        self.warlock.lock('promotion', lockTtl, function (err, unlock) {
            ...
            if (typeof unlock === 'function') {
                ...
                client.select(self._options.redis.db);
...
Queue.prototype.setting = function (name, fn) {
    fn = fn || noop;
    this.client.select(this._options.redis.db);
...
Queue.prototype.process = function (type, n, fn) {
    ...
    while (n--) {
        ...
        worker.on('job complete', function (job) {
            ...
            if (self.client) {
                self.client.select(self._options.redis.db);
...
Queue.prototype.activeCount = function (name, type, fn) {
